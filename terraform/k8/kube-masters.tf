resource "vsphere_virtual_machine" "kube-masters" {
  count = var.kube_config.master_count

  name             = "${var.kube_masters.node_name_prefix}-${format("%02s", count.index + 1)}"
  resource_pool_id = data.vsphere_compute_cluster.cluster.resource_pool_id
  datastore_id     = data.vsphere_datastore.datastore.id
  folder           = var.vsphere_build.folder

  num_cpus = var.kube_masters.cpus
  memory   = var.kube_masters.memory
  ...
  clone {
    ...
    customize {
      linux_options {
        host_name = "${var.kube_masters.node_name_prefix}-${format("%02s", count.index + 1)}"
      ...
      network_interface {
        ipv4_address = cidrhost("${var.kube_network.ipv4_master_network}/${var.kube_network.ipv4_master_netmask}", var.kube_network.ipv4_master_start_address + count.index)
        ipv4_netmask = var.kube_network.ipv4_master_netmask
      }

      ipv4_gateway = var.kube_network.ipv4_master_gateway
    }
  }
  ...
  provisioner "file" {
    ...
    connection {
      ...
      host        = cidrhost("${var.kube_network.ipv4_master_network}/${var.kube_network.ipv4_master_netmask}", var.kube_network.ipv4_master_start_address + count.index)
    }
  }
